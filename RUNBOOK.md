...
| **`report.html` (I6)** | **built 2026-09-11, local gates only.** `rule all` targets it, so a run is not finished until it is readable. Rendered only against a synthetic workspace so far: `tests/report_render.sh` builds a deliberately PARTIAL one and asserts what the page says is MISSING. Never produced from a real `5.analysis/` |
