...
| **Per-rule LSF resources (I5)** | **built 2026-09-11, local gates only.** Every rule names its own memory and wall-clock tier, derived from `tests/measured_resources.tsv`. Before the first cluster run: confirm a node in the queue has the 256000 MB R09 now asks for, and replace R19/R20's UNMEASURED tiers with real figures |
