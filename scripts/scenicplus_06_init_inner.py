...
def main():
    ...
    p.add_argument("--out_dir", required=True, help="Where to scaffold the SCENIC+ snakemake")
    ...
    # `scenicplus init_snakemake` writes scplus_pipeline/Snakemake/{config,workflow}
    subprocess.run(
        ["scenicplus", "init_snakemake", "--out_dir", str(out_dir)],
        check=True,
    )
    ...
    print(f"[init_scenicplus] Patched: {cfg_path}")
